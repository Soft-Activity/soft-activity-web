...
const { tableRegister, tableState, tableMethods } = useTable({
  fetchDataApi: async () => {
    const res = await getMenuList({ query: searchParams.value })
    return {
      list: res.data || []
    }
  }
...
const searchParams = ref<API.MenuQuery>({})
const setSearchParams = (data: API.MenuQuery) => {
...
const currentRow = ref<API.MenuVO>()
...
const save = async () => {
  ...
  // 如果formData有menuId，则表示是编辑,否则是新增
